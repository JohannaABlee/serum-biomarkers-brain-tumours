...
time=linspace(1,1000,1000);%time of growth simulation
...
b=0.01;%growth rate of necrotic regions
maxVt=158;%maximum tumour volume (ml)
maxNt=158*pt; %maximum number of tumour cells
...
yp=0.69;%days-1 decay rate in serum

%kfunction intilaisation
tk0=tn0%inthis scenario assume necrosis and k onset align
Ktmin=0.05;%minimum k value
Ktmax=0.8;%maximum k value
h=7;%rate of k change hill slope constant
Ktt=100+tk0;%half time - when k is half max 


%necrosis constants
Rd=b;%rate of cell death- same as necrosis growth rate
Qn=3*10^-4;%average level of GFAP in necrotic cells

Vp=4500; %ml mean plasma volume


CPss=0.07; %ng/ml average conc in serum of healthy patients
...
[time2,ytn1]=ode45(@(time2,ytn1)((kfunc(tk0,time2,Ktmin,Ktmax,Ktt,h)*Un(Qn,Rd,tn0,time2,maxNn,b,Nn0))+KHUH-ytn1*yp),totaltime,qt0);
